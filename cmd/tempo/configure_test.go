...
func TestGetConfigPath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			configPathVar:  "/custom/path/config.json",
			expectedSuffix: "/custom/path/config.json",
		},
		{
			name:           "uses default path when configPath empty",
			configPathVar:  "",
			expectedSuffix: "/.devctl/plugins/config-tempo.json",
		},
	}
...
func TestInitConfig(t *testing.T) {
	...
	tempDir := t.TempDir()
	testConfigPath := filepath.Join(tempDir, "config-tempo.json")
...
func TestConfigureIssueId(t *testing.T) {
	tests := []struct {
		name        string
		inputId     string
		expectedKey string
		...
		{
			name:        "issue id provided",
			inputId:     "PROJ-123",
			expectedKey: ISSUE_ID,
			expectedVal: "PROJ-123",
		},
		{
			name:        "issue id with whitespace trimmed",
			inputId:     "  PROJ-456  ",
			expectedKey: ISSUE_ID,
			expectedVal: "PROJ-456",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Only test cases where input is provided (to avoid os.Exit)
			if tt.inputId != "" {
				configureIssueId(tt.inputId)
...
func TestFetchConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tempDir := t.TempDir()
			configFile := filepath.Join(tempDir, "config-tempo.json")
			...
			if tt.presetAccountId != "" {
				viper.Set(ACCOUNT_ID, tt.presetAccountId)
			}
			if tt.presetIssueId != "" {
				viper.Set(ISSUE_ID, tt.presetIssueId)
			}
